...
Wrangle the urchins data to describe urchin mean size, median, standard deviation of the mean, and the sample size.
...
Create a table that includes the summary statistics determined in the data wrangling step for the urchins data.
...
urchins_summarized %>% 
  kable(col.names = c("Common Name", 
                      ...
                      "Sample Size (n)"), 
        caption = "Figure 2: Summary Statistics on Size Distribution for Purple and Red Sea Urchins." ) %>% 
...
```{r}
purple_urchins_site_year <- purple_urchins %>% # storing a new data frame from purple_urchins
...
Figure 3 shows purple urchin populations declining across all 5 sites sampled along the Santa Barbara coast. However, some sites were annually treated to remove giant kelp once each winter and allowed to colonize over the course of the year while other sites were left intact. Since Figure 3 does not does not account for the differences in urchin counts between experimental kelp treatments, it would not be representative of actual changes in purple urchin populations over time. In order for the changes in counts by year to be reflective of actual changes in purple urchin populations, we would need to account for the effect of experimental kelp treatment on purple urchin populations.
